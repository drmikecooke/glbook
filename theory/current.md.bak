...
as useful for a boundary where one would like $\mathbf n\cdot\mathbf j=0$. The value $r=0$ would be appropriate for a superconductor boundary condition with a vacuum. Non-zero values could be relevant for boundaries with normal metals. Presumably edges are more complicated in reality, some of which might be modeled using modifications of the GL equations, such as position variable $a,b,K,$ etc.
